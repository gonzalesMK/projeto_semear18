...
void set_eletroima(const projeto_semear::setEletroimaGoalConstPtr &goal, actionlib::SimpleActionServer<projeto_semear::setEletroimaAction> *as, tf::TransformListener &listener)
{
    ...
    double dist_x = 0;// goal_x - pose_msg.linear.x;
    double dist_y = 0;// goal_y - pose_msg.linear.y;
    double dist_z = 0;// goal_z - pose_msg.linear.z;
    ...
    while (!succeed && nh.ok() && ros::ok())
    {
        ...
        dist_x = 0;//goal_x - pose_msg.linear.x;
        dist_y = 0;//goal_y - pose_msg.linear.y;
        dist_z = 0;//goal_z - pose_msg.linear.z;
...
int main(int argc, char **argv)
{
    ...
    actionlib::SimpleActionServer<projeto_semear::moveEletroimaAction> server1(node, "moveEletroima", boost::bind(&move_eletroima, _1, &server1, boost::ref(listener1)), false);
    actionlib::SimpleActionServer<projeto_semear::setEletroimaAction> server2(node, "setEletroima", boost::bind(&set_eletroima, _1, &server2, boost::ref(listener2)), false);
